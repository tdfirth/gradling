...
def test_multi_head_flash_attention_dims(subtests, rngs):
    dims = [
        (1, 4, 8),
        (4, 16, 32),
    ]
    ...
    for dim in dims:
        for nh in num_heads:
            with subtests.test(f"dims={dim}, num_heads={nh}"):
                B, T, C = dim
                xs = random.normal(rngs(), dim)
                attn = MultiHeadAttention(T, C, nh, 0.1, rngs, use_flash_attention=True)
                out = attn(xs)
                assert out.shape == dim


def test_multi_head_flash_attention_future_token_indpendence(subtests, rngs):
    jnp.set_printoptions(linewidth=200, precision=4, suppress=True)
    # If we change some tokens at step t, then the output before t should not
    # be affected.
    nh = 2
    B, T, C = dim = (1, 6, 16)
    attn = MultiHeadAttention(T, C, nh, 0.0, rngs, use_flash_attention=True)
    xs1 = random.normal(rngs(), dim)
    xs2 = xs1.copy()
    out1 = attn(xs1)
...
def test_multi_head_flash_attention_gradient_causality(subtests, rngs):
    nh = 2
    B, T, C = dim = (1, 4, 4)
    attn = MultiHeadAttention(T, C, nh, 0, rngs, use_flash_attention=True)
    xs = random.normal(rngs(), dim)
    J = jax.jacobian(lambda x: attn(x))(xs)
